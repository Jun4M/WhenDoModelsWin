"""
Cross-GPU verification variant of run_learning_curve.py — adds --seeds CLI override.
Used for T4/L4 invariance check. Delete after cross-GPU verification complete.
...
def run_one(
    ...
    if not skip_unimol and not (resume and run_already_done(raw_dir, 'unimol_scratch', None, seed, target, train_size)) and tr['X_unimol'] is not None and len(tr['X_unimol']) > 0:
        try:
...
def parse_args():
    ...
    p.add_argument('--seeds', nargs='+', type=int, default=None,
                   help="Override seed schedule (e.g. --seeds 0)")
    return p.parse_args()
