"""
test_gcn_mtl_determinism.py
Seed determinism: same seed → identical loss curve final value + identical test_preds
for all 12 tasks (ATOL=1e-5).
"""
...
from src.train import train_gcn_mtl
...
def test_seed_determinism():
    """Two runs with the same seed produce identical (N, 12) test_preds."""
    n_mols = 16
    ...
    r1 = train_gcn_mtl(**kwargs, seed=77)
    r2 = train_gcn_mtl(**kwargs, seed=77)

    np.testing.assert_allclose(
        r1['test_preds'], r2['test_preds'], atol=1e-5,
        err_msg="GCN-MTL test_preds differ between runs with the same seed",
    )
