...
def merge_final(
    ...
    try:
        ...
        baselines_df_plot = pd.read_csv(baseline_path) if os.path.exists(baseline_path) else pd.DataFrame()
        depth_df_plot     = pd.read_csv(depth_path)    if os.path.exists(depth_path)    else pd.DataFrame()
        plot_combined_final(baselines_df_plot, best_depth, depth_df_plot, plots_dir, target)
